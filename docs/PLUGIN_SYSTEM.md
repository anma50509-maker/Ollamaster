...
聊天页 → 人设卡 → 管理人设 → **导入酒馆人设卡**（矢量图标入口），支持两种方式：
...
**方式二：选择卡片文件**（对话框内「选择卡片文件」按钮）：调用系统文件选择器，直接选取酒馆角色卡 **`.json`** 或 **`.png`** 文件。PNG 卡自动解析其 tEXt 块的 `chara` 键（兼容明文 JSON 与 base64 两种存放形式），无需手动复制内容。
